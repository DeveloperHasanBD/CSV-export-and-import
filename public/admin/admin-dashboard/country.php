...
?>
	...
	function update_state_name()
	{
		...
		$lagmp_countries_table 		= $wpdb->prefix . 'lagmp_countries';
		$country_slted_id 			= $_GET['country_id'] ?? '';
			...
			<div class="row">
				<div class="col-4">
					<div class="country_form shadow p-3 mb-5 bg-body rounded">
						...
						if ('Update country' == $update_country_btn) {
							$upd_country_name = sanitize_text_field($_POST['country_name']);
							$country_lat = sanitize_text_field($_POST['country_lat']);
							$country_lng = sanitize_text_field($_POST['country_lng']);
							$upd_select_state_name = sanitize_text_field($_POST['select_state_name']);
							$wpdb->update(
						...
						?>
							<form action="" method="POST">
								...
								<input type="submit" name="update_country_btn" value="Update country" class="btn btn-primary mt-2">
								<a class="btn btn-info mt-2" href="<?php echo admin_url("admin.php?page=country"); ?>">Add more country ?</a>
							</form>
						<?php
						} else {
						?>
				...
				<div class="col-8">
					<div class="state_list_table shadow p-3 mb-5 bg-body rounded">
						...
						if ('Delete country' == $delete_country_name) {
						...
						$delete_all_countries_btn 	= $_POST['delete_all_countries_btn'] ?? '';
						if ('Delete all country' == $delete_all_countries_btn) {
							$wpdb->query("TRUNCATE TABLE $lagmp_countries_table");
						?>
							<div class="alert alert-success alert-dismissible fade show" role="alert">
								<strong>Successfully</strong> deleted all data.
								<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
							</div>
						<?php
						}
						...
						<table class="table" id="country_list_table">
							<thead>
								<tr>
									...
									<th scope="col">Country lat</th>
									<th scope="col">Country lng</th>
									<th scope="col"> 

									<div class="action_btn_dgn">
											<span>Action</span>
											<form action="" method="POST">
												<input name="delete_all_countries_btn" class="btn btn-warning" type="submit" value="Delete all country" onclick="return confirm('Are you sure want to delete all ?')">
											</form>
										</div>
								...
								?>
									<tr>
										...
										<td class="action_btn_dgn">
											<a href="<?php echo admin_url("admin.php?page=country&country_id=$country_id"); ?>" class="btn btn-info">Update</a>
											<form action="" method="POST">
												<input type="hidden" name="delete_country_did" value="<?php echo $country_id; ?>">
												<input name="delete_country_name" class="btn btn-danger" type="submit" value="Delete country" onclick="return confirm('Are you sure want to delete ?')">
											</form>
										</td>
									</tr>
								<?php
